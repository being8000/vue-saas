...
import { Editor, Path, Node, Text, BaseElement } from "slate";
...
const ZeroWidthString = defineComponent<ZeroWidthStringProps>({
  setup( props) {
...
const String = defineComponent<{
  ...
  setup(props) {
    const { leaf, editor } = props;
    const isLast = inject('isLast')
    const parent = inject<BaseElement>('parent')!
    ...
    if (
      leaf.text === "" &&
      (parent as Element & Editor).children[
        (parent as Element & Editor).children.length - 1
      ] === text &&
